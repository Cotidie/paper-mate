...
**`annotation-comment-pin`** — A comment both **highlights the underlying text** (accent at ~0.4) **and** anchors a `{rounded.full}` pin (accent fill) to mark it as a comment. Click opens the `comment-bubble`.
