...
### AD-L8, Document-structure extraction (second domain tenant)
- **Binds:** AD-L2 (opens the domain layer), AD-L4 (import pipeline), AD-8 (storage layout), AD-9 (storage sole writer); main-spine **AD-13** (the structure layer this feeds); FR-34, LFR-8
- **Prevents:** structure extraction being bolted onto routes or the client; a JVM/host dependency leaking past the container; the structure artifact contaminating the shared `library.json` write path (AD-L7)
- **Rule:** structure extraction is the **second tenant** of the AD-L2 backend domain layer (after metadata extract/enrich). `domain/structure.py` exposes a swappable port `extract_structure(pdf_bytes) -> DocStructure` with **opendataloader-pdf** (Apache-2.0, Java core via its Python binding) as the first adapter, run **in-container** with a bundled JRE. It runs in the AD-L4 background import pipeline (create-story call: within the existing `extracting` state or a distinct `analyzing` one), and its result is persisted by the storage module (AD-9) as a **per-doc** artifact `~/.paper-mate/library/{doc_id}/structure.json` beside `source.pdf`/`meta.json` (AD-8). Because it is per-doc, not the shared cross-doc index, it does **not** go through the AD-L7 serialized `library.json` write path (no new index-concurrency surface). Extraction is **total** (a failure yields empty structure, never blocks a paper reaching `ready`) and never makes a network call (born-digital local mode; OCR/hybrid deferred). Metadata extraction (AD-L2 `extract()`) MAY consume this structure as its preferred title source, keeping the PyMuPDF heuristic as the fallback (LFR-8). *(Added 2026-07-20 via correct-course; see `sprint-change-proposal-2026-07-20-opendataloader-structure-layer.md`. The consumer/reader side is main-spine AD-13.)*
