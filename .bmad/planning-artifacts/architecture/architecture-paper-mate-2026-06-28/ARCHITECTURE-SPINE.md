...
### AD-13, Document-structure layer `[ADOPTED]`
- **Binds:** FR-3 (ToC), FR-27 (reading-helper preview), FR-34/35/36/37 (structure, section nav, figures/tables index, digest); LFR-8 (metadata); all Phase-2 reading-helper surfaces + Phase-3 paper-digest context. Amends the Deferred "Phase 2 surfaces … not designed here."
- **Prevents:** four hand-rolled, drifting PDF-interpretation detectors (title-from-fonts heuristic, embedded-outline-only ToC, regex-over-spans reading-helper, figure/table geometry) each re-deriving structure; coordinate math leaking outside the anchor boundary.
- **Rule:** the reader is **section-aware** from ONE server-produced, box-anchored document-structure model. A backend extractor (`domain/structure.py`, AD-L8) runs **opendataloader-pdf** (Apache-2.0) at **import** (AD-L4 background pipeline) and persists a per-doc `structure.json` (AD-8 layout) of typed elements `{ id, type ∈ heading|paragraph|table|figure|caption|list|footnote|…, page_index, rect, text, heading_level? }`. opendataloader boxes (`[left,bottom,right,top]` PDF points, 1-indexed page) are converted **server-side** to the AD-4 normalized `Rect` (`[0,1]`, top-left, y-down) so the layer IS anchors, the client `structure/` service denormalizes to screen at the current scale exactly like the annotation anchor service, and **all** structure→screen math stays at that anchor boundary (AD-9). Exposed via `GET /api/docs/{id}/structure` (AD-3 generated types). Consumers (synthesized ToC, Figures/Tables index, reading-helper previews, structure-backed metadata, Phase-3 digest) are **thin readers** of this layer; the heuristics they replace survive only as graceful fallbacks when structure is thin. Extraction is behind a swappable port (opendataloader the first adapter), total (a failure yields empty structure, never blocks `ready`). It runs in one of **two runtime-switchable modes** (`PAPER_MATE_STRUCTURE_MODE`, Story 10.3): **local/fast** (born-digital, deterministic + offline; **the default**, Story 10.1) or **hybrid** (a bundled Docling+EasyOCR *hybrid server* — a SEPARATE process the opendataloader Java core calls over HTTP — for higher structure fidelity: it recovers section headings the local heuristic mis-tags/drops). Both modes emit opendataloader's SAME JSON tree (spike-confirmed byte-identical shape), so the server-side points→`Rect` mapping and the `DocStructure` contract are unchanged — a consumer cannot tell which mode produced a structure. The "deterministic + offline" guarantee is **mode-dependent**: local preserves it (and stays the offline default); hybrid's models are **baked into the image at build** so it too runs offline (NFR-1 satisfiable in both modes), and it is deterministic within a device (sub-pixel coordinate jitter across CPU/GPU, invisible after normalization). Runs **in-container** (a JRE + the `opendataloader-pdf` Python binding always; plus the `[hybrid]` deps — docling[easyocr]+torch, multi-GB — and the launched hybrid server when in hybrid mode; the PDF is already in `/data`), unlike the Phase-3 host agent CLIs it does **not** hit the dockerization boundary. The image is **always hybrid-capable** (deps + baked models bundled), so switching is an env flip + a container restart with **no rebuild**; the hybrid server is launched from the FastAPI lifespan **only in hybrid mode** (local pays no runtime cost). Hybrid is **GPU-optional with a CPU fallback** (`PAPER_MATE_STRUCTURE_HYBRID_DEVICE`, default `auto`): a GPU-less container still works. *(Added 2026-07-20 via correct-course, `sprint-change-proposal-2026-07-20-opendataloader-structure-layer.md`; hybrid mode un-deferred 2026-07-21 and prioritized to **Story 10.3**, `sprint-change-proposal-2026-07-21-structure-hybrid-mode.md` + `-prioritize-hybrid-mode.md`. Extraction home = library spine AD-L8.)*
...
| FR-34..37 structure/ToC/figures-tables/digest; FR-27 reading-helper | server `domain/structure.py` + `storage/` (`structure.json`) → client `structure/` service (thin consumers) | AD-13, AD-L8, AD-4, AD-9 |

## Deferred

- **Phase-3 agent execution boundary**, a dockerized backend cannot exec host agent CLIs (Claude/Codex/Antigravity). Resolve at Phase 3 (host bridge / mounted docker socket / agents-in-image / sidecar). AD-9's `server/agents/` reserves the seam; the agent abstraction must not assume same-process exec. *(Note: the AD-13 opendataloader extractor runs in-container and does NOT hit this boundary, it processes a file already in `/data`, not a host CLI.)*
- **Phase 2 surfaces**, ~~inline reference previews, metadata extraction~~ **now designed by AD-13 (document-structure layer, Epic 10)**; export-with-highlights (FR-26, Epic 12) and the Library page (shipped, Epics 6-7) remain their own tracks. All consume AD-4 anchors.
- **OCR / scanned PDFs** remains deferred. opendataloader's **hybrid mode** (a bundled Docling+EasyOCR server), by contrast, is **adopted (Story 10.3, un-deferred 2026-07-21)** as a runtime-switchable extraction mode for higher structure fidelity; local mode stays the deterministic + offline default (see AD-13).
