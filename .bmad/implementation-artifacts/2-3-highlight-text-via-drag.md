...
- [x] [Render][Investigate] **Highlight overflows into the right margin (not reproduced on the current build).** Measured the rendered rects vs glyph extent live at 157% AND 246%: bands are tight to the text — justified lines reach the margin (correct), the ragged short line got a tight rect, zero-width phantom rects are already filtered. The reporting screenshot predates the anchor/zoom corrections (2.2 review + this session's fixes); the `mergeRects` change further hardens it. Owner to re-verify on the current build; if it recurs, capture a fresh repro.

- 2026-06-29 (Story 2.3, render-fidelity): fixed live text-selection invisibility (override pdf.js's transparent `::selection`) and highlight opacity-stacking/thickening (per-line `mergeRects` + an `.annotation-highlights` opacity group so overlaps don't compound and recent wins). Right-margin overflow not reproduced on the current build (rects tight at 157%/246%); hardened via mergeRects, owner to re-verify. Tests added (mergeRects ×3). Frontend 205 pass, typecheck clean. Verified live with Playwright.
