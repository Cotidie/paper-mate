...
- **Code review (bmad-code-review via Codex, 2026-07-01)** — 0 AC violations, 2 patch findings, both resolved: (1) `read_annotations` now rejects duplicate ids and foreign `doc_id` entries as `CorruptAnnotationsError` (a duplicate would be silently collapsed by the client's id-keyed Map, a foreign entry would restore into the wrong doc — NFR-4/AC-5); +2 backend tests. (2) The anti-clobber App test now enables fake timers BEFORE render/open so a debounce scheduled during open is actually driven (a real-timer PUT would otherwise fire after the test and falsely pass). Backend 67 + frontend 524 green after fixes.
