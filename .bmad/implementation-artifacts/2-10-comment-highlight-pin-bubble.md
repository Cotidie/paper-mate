...
> **Comment is NOT a new geometry — it is a `type=comment`, `kind=text` mark (a highlight run) that ALSO grows a round pin and a note bubble.** Highlight/underline are `kind=text` built from a drag-selection; pen is `kind=path`; memo is `kind=rect` placed by a click. Comment REUSES the highlight drag-selection path verbatim: a drag across text releases a `kind=text` annotation, but with `type="comment"` and a non-null `body`. The genuinely new pieces are tiny: (1) the highlight FILL is FREE (a `type=comment` mark is `type !== "underline"`, so it already paints in the `.annotation-highlights` 0.4-opacity group), (2) a round PIN rendered at the start of the run (a focusable control, so it lives in a NEW non-aria-hidden `.annotation-comments` group), and (3) a `{component.comment-bubble}` that opens on pin-click for read/edit of `body` (the comment twin of memo's interactive `<textarea>`, reusing `retextAnnotation` unchanged). Everything else is the SAME pattern the tool stories already set: a rail button below Memo + a `C` hotkey + an arm-time color sub-toolbox, the create-on-release-then-select flow, and `denormalizeRect` zoom-glue. The contract ALREADY supports comment (`type:"comment"` in the `Annotation` union, `kind=text`, `body: string | null`), so this is client-only with the tracked contract byte-identical.

## The decisions that define this story (read before coding)

**1. A comment is created by a DRAG across text (the highlight gesture), NOT a click or a placement.** With comment armed (rail button or `C`), drag-selecting text and releasing creates ONE `kind=text` annotation per page (the SAME `buildAnnotations` two-page `group_id` split as highlight), but `type="comment"` and `body=""`. The ONLY delta from the highlight create path is `type` + a non-null `body`. The mark lands, is selected, and its bubble opens for immediate typing. The whole gesture reuses the existing document-level `pointerup` selection path in `AnnotationInteraction` (NOT the pen drag or the memo click). [Source: epics.md#Story-2.10 AC1; UX-DR5 (comment mode); DESIGN.md#annotation-comment-pin]

**2. The highlight FILL is FREE; the new render is the PIN + the BUBBLE.** A `type=comment` mark is `kind=text` and `type !== "underline"`, so `AnnotationLayer`'s existing `highlightMarks` filter ALREADY paints it as a ~0.4 accent fill in the `.annotation-highlights` opacity group (UX-DR7). Do NOT add a second fill path. NEW = a round `{component.annotation-comment-pin}` (rounded.full, ~20px, accent fill) anchored at the START of the run's first rect, rendered as a `<button>` in a NEW, NOT aria-hidden `.annotation-comments` group (a focusable control cannot live in the decorative aria-hidden mark sheet — same rule memos follow). The pin rides the denormalized first rect so it stays glued on zoom (NFR-3). [Source: DESIGN.md#annotation-comment-pin; AnnotationLayer.tsx (highlightMarks already includes comment)]
...
**5. An empty comment is KEPT (unlike an empty memo).** A memo placed but never typed into is a stray empty box, so 2.9 auto-removes it on deselect. A comment is born from a deliberate text drag — the highlight+pin IS the annotation and the note is optional. So there is NO empty-comment cleanup: deselecting a comment with empty `body` leaves the highlight+pin in place. (This is the one place the memo pattern does NOT carry over — call it out so the dev does not copy the 2.9 cleanup effect.)

## Scope boundary — READ FIRST

**IN (this story):**

- **Comment create on drag-release.** In `AnnotationInteraction.tsx`, extend the existing `pointerup` selection branch so `tool === "comment"` builds a `kind=text` mark like highlight/underline but with `type="comment"` + `body=""`, adds it, and selects it (which opens the bubble). Comment is a text-selection tool, so it MUST stay OUT of the pen/memo early-return (`armedToolRef.current === "pen" || "memo"`); add `comment` to the create branch alongside `highlight`/`underline`.
- **`buildAnnotations` gains an optional `body` (NEW param, default `null`).** Extend `BuildOptions` with `body?: string`; map it onto each built annotation (highlight/underline keep the default `null`; comment passes `""`). Pure, unit-tested. (Do NOT fork a `buildCommentAnnotations` — comment IS the text path with a body.)
- **Comment pin render (NEW group).** `AnnotationLayer.tsx`: a `type=comment` branch rendering a round `{component.annotation-comment-pin}` `<button>` at the start (top-left) of the comment's FIRST rect, positioned via `denormalizeRect` + the card box, in a NEW, NOT aria-hidden `.annotation-comments` group (pointer-transparent group, the pin opts back in). The pin's fill is `var(--color-${style.color})`; clicking it `select`s the comment. `data-testid={`annotation-comment-pin-${a.id}`}` and the existing `annotation-mark-${a.id}` testid stays on the FILL (the highlight group already paints it). The pin re-derives on zoom (NFR-3).
...
- **A `kind=rect` comment** (a comment pinned to a region, not a text run). AD-5 allows `comment → text` OR `rect`; this story builds the `text` path (the AC's "drag across text"). The `rect` comment is a future refinement.
...
1. **Comment armed → drag across text + release highlights the run (~0.4) AND anchors a round `{component.annotation-comment-pin}` at the spot (epics.md#Story-2.10 AC1; FR-11, UX-DR7).** With comment armed (rail button or `C`), drag-selecting text and releasing paints a ~0.4 accent fill OVER the run (the existing `.annotation-highlights` group, since `type=comment` is `type !== "underline"`) AND renders a round accent pin at the start of the run. Nothing reflows (NFR-1). [Source: epics.md#Story-2.10; DESIGN.md#annotation-comment-pin, #annotation-highlight; AnnotationLayer.tsx; AnnotationInteraction.tsx (create branch)]
...
3. **The comment stores as `type=comment`, `anchor kind=text` (rects + text), `body=text` non-null (epics.md#Story-2.10 AC3; AR-5).** The drag builds `Annotation {type:"comment", anchor:{kind:"text", page_index, rects, text}, style:{color}, body}`; `body` is non-null (starts `""`, updated as the user types). A two-page comment shares one `group_id` (the existing `buildAnnotations` split). [Source: epics.md#Story-2.10; ARCHITECTURE-SPINE.md#AD-5 (`comment → text` or `rect`; `body` non-null for memo/comment); create.ts (buildAnnotations + body)]

4. **The highlight and pin stay anchored across zoom (epics.md#Story-2.10 AC4; NFR-3).** After creating a comment, zooming re-renders the fill AND the pin at the exact page location and scale (both ride the denormalized rects via `denormalizeRect`); screen position derived, never persisted. Prove LIVE at DPR>1. [Source: epics.md#Story-2.10; ARCHITECTURE-SPINE.md#AD-4; AnnotationLayer.tsx]

5. **Comment is a first-class tool in the single `activeTool` FSM (AD-11).** The rail has a Comment button below Memo (DESIGN.md#tool-rail order); clicking it switches `activeTool` to `"comment"` in ONE click; `C` arms it; `V`/`Esc` returns to cursor; arming comment disarms whatever was active (mutual exclusion). Switching TO comment auto-opens its color sub-toolbox (reusing `ToolFlyout` + the shared `flyoutOpen` + the open-on-tool-change effect). A click on the already-active button toggles it; `Esc`/outside-click/switch-away/collapse close it. While comment is armed, pan is off and a drag selects text to comment (not a pan). [Source: ARCHITECTURE-SPINE.md#AD-11; tools.ts (`ANNOTATION_TOOLS` includes "comment"); UX-DR4/DR15; ToolRail.tsx; Story 2.6]
...
- [ ] **Task 1 — buildAnnotations body param (AC: 1, 3)**
  - [ ] `client/src/annotations/create.ts`: add optional `body?: string` to `BuildOptions` (default `null`); set `body: body ?? null` on each built annotation. Highlight/underline callers unchanged (omit it → `null`); comment passes `""`. Update the doc comment (no longer "Story 2.2 proof = highlight" only).
  - [ ] `create.test.ts`: `buildAnnotations` with `type:"comment"`, `body:""` → `body === ""`; without `body` → `body === null` (highlight/underline regression).

- [ ] **Task 2 — Comment create gesture (AC: 1, 3)**
  - [ ] `client/src/annotations/AnnotationInteraction.tsx`: in the `pointerup` handler, add `comment` to the text-anchor create branch (`tool === "highlight" || tool === "underline" || tool === "comment"`), passing `type: tool` + `body: tool === "comment" ? "" : undefined`. Confirm comment is NOT caught by the `pen`/`memo` early-return (it reads the text selection). After create, `select(created[0].id)` (opens the bubble).
...
- [ ] **Task 3 — Comment pin + bubble render (AC: 1, 2, 4, 6)**
  - [ ] `client/src/annotations/AnnotationLayer.tsx`: a `type=comment` render path. The FILL stays in `.annotation-highlights` (comment is already in `highlightMarks` — verify, do not double-paint). Add a NEW, NOT aria-hidden `.annotation-comments` group rendering, per comment mark: a round pin `<button>` at the denormalized first-rect start (`data-testid="annotation-comment-pin-${a.id}"`, accent fill, `onClick` → `select`), and when selected, a `CommentBubble`.
  - [ ] `CommentBubble` component (NEW, extracted like `MemoBox`): a `<textarea>` (`value=body`, `onChange`→`retextAnnotation`, autofocus when selected, local `Esc` → blur + `clearSelection`), a `ColorSwatchRow` (recolor → `recolorAnnotation` + set default), a delete button; anchored below the pin via the denormalized first rect; height re-fits on body/scale change (the `MemoBox` layout-effect pattern). Owns its ref.
...
- [ ] **Task 5 — Tests + regression bar (AC: all)**
  - [ ] `create.test.ts`: body param (Task 1).
  - [ ] `AnnotationInteraction.test.tsx`: comment drag creates `type=comment`/`kind=text`/`body=""`, selects it; comment NOT in the pen/memo early-return; selecting a comment does NOT open the generic selection quick-box (`showSelectionBox` excludes comment); clicking the pin does not clear selection. Highlight/underline/pen/memo/cursor paths still pass.
  - [ ] `AnnotationLayer.test.tsx`: a `type=comment` mark paints a fill in `.annotation-highlights` AND a pin in `.annotation-comments` (not aria-hidden); the pin is a button with the testid; selecting it renders `CommentBubble` with `value=body`; typing fires `retextAnnotation`; recolor fires `recolorAnnotation`; delete fires `deleteAnnotation`; re-derives on zoom; empty comment is NOT auto-removed.
  ...
  - [ ] **Live smoke** (own fresh `uvicorn` + `vite dev` on alternate ports, a real PDF, DPR=2): (a) arm Comment from cursor (one click) → flyout opens with color; (b) `C` arms comment; (c) drag across text → run highlighted ~0.4 + a round pin at the start; (d) click the pin → bubble opens focused, type a note (body persists), `Esc` dismisses + focus returns; (e) re-click the pin → bubble re-opens with the saved note; (f) recolor from the bubble → fill + pin retint; (g) delete from the bubble removes the comment; (h) zoom 200→250% → fill + pin glued (record fracLeft/fracW invariants); (i) an empty comment (drag, no typing, deselect) STAYS; (j) highlight/underline/pen/memo still create + select; (k) CROSS-PAGE comment drag at DPR>1 (the highest-risk selection path — fill must not leak full-page; pin renders on the start). Screenshot to `.bmad/implementation-artifacts/2-10-comment-smoke.png`.
...
| Need | Reuse | New |
| --- | --- | --- |
| Text-anchor create + two-page `group_id` split | `buildAnnotations` + the `pointerup` selection path | add `comment` to the create branch; a `body` param |
| Entity + contract | `type:"comment"`/`kind=text`/`body` already generated | nothing (client-only) |
| Highlight FILL (~0.4) | the `.annotation-highlights` opacity group already includes `type !== "underline"` | nothing — comment paints for free |
...
Resist: a `buildCommentAnnotations` fork (comment IS the text path + a body); a second fill path for comment (it is already in `highlightMarks`); a bespoke body store action (reuse `retextAnnotation`); copying the 2.9 empty-memo cleanup onto comments (Decision 5 — comments are kept); putting the pin in the aria-hidden mark sheet (it is a focusable control → the `.annotation-comments` group, like memos); a `kind=rect` comment (text path only this story).
...
- `client/src/annotations/create.ts` — `BuildOptions.body?` (default null) threaded onto each built annotation.
- `client/src/annotations/AnnotationInteraction.tsx` — add `comment` to the `pointerup` text-anchor create branch (body `""`); exclude `type==="comment"` from `showSelectionBox`; add `.annotation-comment-pin` to the two `.closest()` hit-tests + the on-mark/open checks. The comment selection shows the bubble (rendered in the layer), so the interaction layer mostly just creates + selects.
...
- [Source: .bmad/planning-artifacts/epics.md#Story-2.10] — story + ACs (drag across text → highlight ~0.4 + a round comment pin; click pin → comment-bubble for read/edit, keyboard-reachable + Esc-dismissable + focus in/out; stores type=comment/kind=text/body; highlight+pin stay anchored on zoom).
...
- [Source: .bmad/implementation-artifacts/2-3-highlight-text-via-drag.md] — the highlight drag-selection create path comment reuses verbatim (type is the only delta, + body).
...
Ultimate context engine analysis completed - comprehensive developer guide created. Comment is the LEAST-new tool of Epic 2: it is a `type=comment`, `kind=text` mark (a highlight run) that reuses the highlight drag-create path verbatim (type + a non-null `body` the only delta), paints its ~0.4 fill for FREE in the existing highlights group, and reuses `retextAnnotation`/`recolorAnnotation`/`deleteAnnotation` with zero store change. Five design calls are pre-resolved (1 — created by a text drag, not a click; 2 — the fill is free, new = a pin + a bubble; 3 — body via the reused `retextAnnotation`, creation-time + on-reopen, command-path is 3.1; 4 — the comment-bubble REPLACES the generic selection quick-box per UX-DR5; 5 — empty comments are KEPT, inverting the 2.9 memo cleanup). The contract already carries comment (`type:"comment"`/`kind=text`/`body`), so this is client-only with the tracked contract byte-identical. Success = comment is a first-class tool (rail button below Memo + `C` + a color sub-toolbox), a drag highlights the run + drops a round pin, clicking the pin opens a keyboard-reachable, Esc-dismissable, focus-managed bubble you read/edit, it stores `type=comment`/`kind=text`/`body`, the bubble recolors/deletes it, the highlight+pin stay glued across zoom, empty comments survive, and the live smoke passes create + pin→bubble + typing + recolor + delete + zoom-glue + cross-page at DPR>1 without regressing highlight/underline/pen/memo/pan/zoom.
