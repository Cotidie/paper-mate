...
- [x] **Task 6: Docs, version, close-out (AC: #6, #7).**
  ...
  - [x] Cross-model Codex review (AE-6) on the diff (`eba70f5..HEAD`); resolve High/Med before done. **Completed.** 2 findings (no Lows), both resolved:
    - **High** — `client/src/useAutosave.ts`: a stale in-flight PUT from a previous `docId` could survive a doc switch (the scheduler refs were shared, not doc-scoped), letting a late response write the wrong doc's snapshot or clear `inFlightRef` while the NEW doc's own PUT was genuinely in flight (breaking H6 across a switch). Unreachable via the shipped UI today (no doc-switch affordance exists pre-Phase-2), but a real latent gap. **Fix:** added `generationRef`, bumped in the doc-reset effect's cleanup (covers both a doc switch and unmount); every async `.then`/`.catch`/`.finally` captures its generation at schedule time and no-ops if it's gone stale. New regression test: `useAutosave.test.ts` "a stale in-flight PUT from a previous doc cannot corrupt the new doc's single-flight state".
    - **Medium** — `server/app/storage/__init__.py` / `routes/docs.py`: `_atomic_write` could raise a raw `OSError` (disk full, permissions) that bypassed the route's `StorageError` → `{detail}` envelope mapping (AR-11), falling through to FastAPI's generic 500. Pre-existing gap in the shared primitive (`_write_meta`/`import_pdf` had it too); 3.4 just added a new caller. **Fix:** `_atomic_write` now wraps `OSError` (from `mkdir`/`mkstemp` and from the write/rename) as `StorageError`, preserving the existing temp-file cleanup for every exception type. New regression test: `test_docs.py::test_put_annotations_disk_failure_returns_500_envelope`.
...
- 2026-07-01: Cross-model Codex review (AE-6) completed and both findings fixed: High (stale in-flight PUT could survive a `docId` switch and corrupt the new doc's single-flight state — `useAutosave.ts` generation guard) and Medium (a raw filesystem `OSError` from `_atomic_write` could bypass the route's `{detail}` error envelope — wrapped as `StorageError`). 2 new regression tests added (1 backend, 1 frontend); full suites still green (54 backend / 514 client), typecheck clean.
