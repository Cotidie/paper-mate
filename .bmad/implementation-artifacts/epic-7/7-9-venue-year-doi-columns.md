...
- server/app/domain/extract.py
- server/app/domain/arxiv_enrich.py (new)
...
- **2026-07-08 (user fix request, post-review):** Venue/Year now fall back to arXiv when Crossref has no venue for a paper AND the PDF carries an arXiv id (a stamp like `arXiv:2103.12345v2` found in the extracted text; `extract.py` gains `_find_arxiv_id`, mirroring `_find_doi`; `ExtractedMeta` gains a transient, non-persisted `arxiv_id` field). New module `server/app/domain/arxiv_enrich.py` (`ArxivFetcher` port + `ArxivEnricher`, mirrors `crossref.py`'s shape) queries arXiv via the maintained `arxiv` PyPI client library (lukasschwab/arxiv.py, adopted per CLAUDE.md over hand-rolling Atom-XML parsing; pinned `arxiv==4.0.0`) rather than raw httpx. `enrich.py` composes it as a second, conditional layer after Crossref: fires only when Crossref left `venue` unset, never overwrites a value Crossref (or the extraction) already found. A successful arXiv fill turns a Crossref `"skipped"` into a real `ready` settle. Two more user fix requests folded into the same fallback branch: (1) DOI - arXiv's own self-assigned DOI is deterministic (`10.48550/arXiv.<id>`, confirmed live: `https://doi.org/10.48550/arXiv.<id>` redirects to the abstract page for every id checked), derived directly with no extra network call, only fills when the PDF/Crossref left `doi` unset; (2) Authors - arXiv's own author list fills in when extraction/Crossref left `authors` empty. Backend suite: 265 passed (10 new: arXiv-id extraction, `_parse`-free `ArxivEnricher.fetch`, `arxiv_doi()`, and the `enrich()` composition branch, each fired/not-fired/failure case). Live-smoked twice on scratch servers with a real arXiv-only fixture (`1907.10211v1.pdf`, no Crossref record): confirmed direct `ArxivEnricher.fetch()` and the full `extract`+`enrich` pipeline against the REAL Crossref + arXiv APIs, then end-to-end through a fresh server + browser (title, authors "Yi Zhu, Shawn Newsam", venue "arXiv", year 2019, doi `10.48550/arXiv.1907.10211` all populated and rendered correctly, column order intact).
- **2026-07-08:** PR #59 merged to `main` (`5f4afbb`). Status set to done.
