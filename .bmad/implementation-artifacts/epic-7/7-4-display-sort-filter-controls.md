...
**Descope note (2026-07-06, post-review, user request):** the Filter control (AC-4, the filter clause of AC-5, `FilterMenu`, `ColumnFilter`/`applyColumnFilter`) was removed after this story shipped for review - "not needed right now." Display + Sort ship as built; ACs/Tasks below are left as originally written for audit history, with strikethrough-equivalent notes marking what no longer applies. See Change Log for the removal commit.
...
4. ~~**(Filter) Narrow rows by a column value.**~~ **DESCOPED 2026-07-06** (user request, post-review) - the Filter control, `FilterMenu`, `ColumnFilter`, and `applyColumnFilter` were removed. Given the Filter control, when I set a column and a value, then only rows whose that-column value matches (case-insensitive substring on the displayed text) remain; clearing the value removes the filter and all rows return. The count line reflects the filtered set ("N files in ..."). One active column filter at a time is sufficient (the AC is "a column value," singular). (LFR-6, L-UX-DR3)

5. **(Sort does not clear selection.** Given a multi/range selection, a sort change PRESERVES it (every row is still visible, only reordered) and 7.3's anchor re-ranges over the new order from the same pivot doc. ~~The filter-clears-selection half of this AC is DESCOPED with the Filter control (2026-07-06).~~ (L-UX-DR2 reset, protects AL-7 batch-move correctness)

6. **(No reflow on open.** Given either remaining control (Display, Sort), when I open it, then its popover overlays (portaled, `position: fixed`, like `MoveMenu`) and does NOT push the table or the page floor down. (L-UX-DR3 "opening any of them never reflows the table or the page floor")

7. **(Scale) No visible stall.** Given hundreds of papers, when I sort, then the result appears without a visible multi-second freeze (LNFR-4). Plain array `sort` over the display-cache rows is comfortably within this; do not add virtualization for it.

8. **(A11y) Controls are keyboard-operable with visible focus.** Given the two remaining controls and their menu items, then each is a real focusable `<button>`, keyboard-operable, with a visible 2px `{colors.ink}` focus ring, and each popover is Esc-dismissable returning focus to its trigger (mirror `MoveMenu`). (L-UX-DR12)
...
- Two toolbar controls: **Display** (column-visibility checklist), **Sort** (column + direction, with a header caret indicator). ~~**Filter** (column + value)~~ - DESCOPED 2026-07-06.
...
- The client-only view-state (`hiddenColumns`, `sort`) and its wiring into `LibraryPage`'s existing `papers → filterPapers → rows` pipeline.
...
- [x] **Task 1 — Column-descriptor model + pure transforms in a new `tableView.ts` leaf (AC: 1, 2, 3, 4)**
  - [x] Create `client/src/library/tableView.ts` (mirror `folderFilter.ts`: a pure, dependency-light leaf module with its own `.test.ts`). Export:
    ...
    - `type SortDirection = "asc" | "desc"`, `interface SortState { column: ColumnKey; direction: SortDirection }`. ~~`interface ColumnFilter { column: ColumnKey; query: string }`~~ - DESCOPED 2026-07-06.
    ...
    - ~~`applyColumnFilter(...)`~~ - DESCOPED 2026-07-06 (removed with the Filter control).
...
- [x] **Task 2 — View-state owner: a `useTableView` hook (AC: 2, 4, 5)**
  - [x] Add `client/src/library/useTableView.ts` (matches the `useFolders`/`useInlineEdit`/`useMovePapers` hook pattern already in the dir). It owns two pieces of client-only state — `hiddenColumns: Set<ColumnKey>` (Title can never enter it), `sort: SortState | null` (~~plus `filter: ColumnFilter | null` - DESCOPED 2026-07-06~~) — and exposes setters plus a `visibleColumns` derivation. Keep it presentational/pure state; it does NOT fetch or persist.
  - [x] Expose the composed transform so `LibraryPage` applies it in one place: given the already-folder-filtered array, produce `sortRows` (~~`applyColumnFilter` → `sortRows`~~ - DESCOPED 2026-07-06). (Column visibility does NOT change the row array, only which cells render — pass `visibleColumns`/`hiddenColumns` + `sort` down to `CollectionTable`.)

- [x] **Task 3 — Wire the pipeline + count + selection-clear in `LibraryPage` (AC: 4, 5, 7)**
  - [x] In `LibraryPage.tsx`, extend the existing derivation. Today: `const visiblePapers = filterPapers(papers, selection)`. New: fold in sort from `useTableView` so `visiblePapers` is the FINAL array handed to `CollectionTable` as `rows` (memoize with `useMemo` keyed on `papers`, `selection`, `sort` for AC-7). The count line already reads `visiblePapers.length` — it will correctly reflect this with no extra change.
  - [x] ~~**Filter change clears the selection (AC-5)**~~ - DESCOPED 2026-07-06 with the Filter control (`handleFilterChange` removed from `LibraryPage.tsx`). Do NOT clear selection on a sort change or a column-hide (rows stay visible). 7.3's `anchorRef` reset already keys off `selectedIds` emptying.
  - [x] Place the controls in the existing `library-toolbar__actions` div, reusing `.toolbar-button` chrome. Order: `Display  Sort | Move  Add`. Do not restructure the toolbar row.
...
- [x] **Task 5 — The control components (AC: 1, 2, 4, 6, 8, 9)**
  - [x] Build under `client/src/library/TableControls/` (scaffold-react colocation, as Stories 5.4/6.8 established): `DisplayMenu.tsx`, `SortMenu.tsx` (~~`FilterMenu.tsx`~~ - DESCOPED 2026-07-06, file removed), a shared `TableControls.css`, and colocated `.test.tsx` files. Each is a `.toolbar-button` trigger + a portaled `position: fixed` popover.
  ...
  - [x] ~~`FilterMenu`: ...~~ - DESCOPED 2026-07-06, file removed.
...
- [x] **Task 6 — Tests (AC: 1, 2, 3, 4, 5, 6, 8)**
  - [x] `tableView.test.ts` (mirror `folderFilter.test.ts`): `sortRows` asc/desc per column incl. the Added-is-chronological-not-lexical case, null-title uses filename fallback, null/empty sorts last, input not mutated. ~~`applyColumnFilter` cases~~ - DESCOPED 2026-07-06, removed.
  ...
  - [x] `LibraryPage.test.tsx` (extend, mirror the existing folder-filter + Move regressions): choosing a sort reorders the rendered rows; a Shift+click range AFTER a sort selects the visually-contiguous run and toolbar Move moves that set (guards the 7.3-follows-sort contract). ~~Filter narrow/clear-selection cases~~ - DESCOPED 2026-07-06, removed. Keep `getLibrary`/`movePapers` mocked; touch no `render/` mock barrel (this is Library, not Reader).
...
- [x] **Task 7 — Live smoke (own fresh servers) (AC: 1, 2, 4, 5, 6, 8)**
  - [x] Launch your OWN fresh `uvicorn` + `vite dev` on alternate ports against an isolated scratch `PAPER_MATE_DATA` (do NOT reuse a user-running server — CLAUDE.md). Seed several real sample PDFs from `fixtures/sample-pdfs/` (there are 8+: `0616.pdf`, `1903.03295v2.pdf`, `DeepAnT_...pdf`, `Microsoft COCO...pdf`, etc.) via the real `POST /api/docs` import path so Title/Authors/Added vary enough to sort meaningfully. Tear both servers down after.
  - [x] Verify live: (a) hiding Authors/Added/File type omits the column and the folder panel + toolbar + page floor do NOT jump; (b) sort asc then desc on each sortable column reorders rows with the correct caret, and reload returns to response order (not persisted); (c) opening each control's popover does not push the table/floor (portaled); (d) each control is keyboard-reachable (Tab) and Esc-dismissable; (e) **7.3 interplay:** after sorting, a plain-click + Shift+click selects the visually-contiguous run and toolbar Move moves the whole set (confirm via `GET /api/library`). Normal DPR is fine — this story adds no coordinate/anchor geometry, so no DPR>1 gate (unlike selection-geometry stories). ~~Filter live-smoke checks~~ - DESCOPED 2026-07-06.
...
**New (client):**
- `client/src/library/tableView.ts` - `ColumnKey`/`ColumnDef`/`COLUMNS`, `SortState`, `sortRows`. (`ColumnFilter`/`applyColumnFilter` removed 2026-07-06, DESCOPED.)
- `client/src/library/tableView.test.ts` - unit tests for `sortRows`.
- `client/src/library/useTableView.ts` - the view-state hook (`hiddenColumns`, `sort`, `visibleColumns`, `applyTableView`).
...
- `client/src/library/TableControls/DisplayMenu.tsx`, `SortMenu.tsx`, `TableControls.css` - the toolbar controls. (`FilterMenu.tsx` removed 2026-07-06, DESCOPED.)
- `client/src/library/TableControls/DisplayMenu.test.tsx`, `SortMenu.test.tsx` - colocated unit tests. (`FilterMenu.test.tsx` removed 2026-07-06, DESCOPED.)
...
- `client/src/library/LibraryPage.tsx` - `useTableView` wiring, `applyTableView` folded into the `visiblePapers` pipeline, the controls added to the toolbar. (Filter wiring - `handleFilterChange`, the filter-gated `visiblePending` guard, `FilterMenu` render - removed 2026-07-06, DESCOPED.)
- `client/src/library/LibraryPage.test.tsx` - Display/Sort integration tests incl. the 7.3-follows-sort Shift+click + Move regression. (Filter test cases removed 2026-07-06, DESCOPED.)
...
- `client/src/library/FolderPanel/FolderPanel.css` - fix request (2026-07-06): `.folder-panel__row-actions` converted from a static flex sibling to an absolute overlay (matching `CollectionTable`'s Open-button pattern), so the idle folder name gets the row's full width instead of the icons permanently reserving theirs.
- `client/src/library/CollectionTable/CollectionTable.tsx` - fix request (2026-07-06): a document-level Shift/Ctrl/Cmd keydown listener blurs a stale native focus left on the Title/Authors cell by a plain click, closing a gap the CSS-only `outline: none` suppression didn't (the underlying DOM focus persisted even though the ring was hidden).
- `client/src/library/CollectionTable/CollectionTable.test.tsx` - 3 regression tests for the modifier-keydown blur guard.
...
- 2026-07-06: Fix request (user-reported): a plain click's native mousedown-focus on the Title/Authors cell persisted through arming, so a LATER bare Shift/Ctrl/Cmd keydown (no new click) could re-flip Chromium's `:focus-visible` heuristic - the existing CSS `outline: none` masked the ring but never removed the underlying focus. Added a document-level modifier-keydown listener that blurs the cell at the source. Also fixed FolderRow's rename/add-subfolder/delete icons reserving static flex width even at `opacity: 0` (permanently shrinking the folder name's ellipsis space); converted to an absolute overlay matching the Open-button pattern. Full suite re-verified (1137 tests), typecheck clean.
- 2026-07-06: DESCOPED the Filter control (user request, "not needed right now"): removed `FilterMenu.tsx`/`.test.tsx`, `ColumnFilter`/`applyColumnFilter` from `tableView.ts`, `filter` state from `useTableView`, and all Filter wiring/tests from `LibraryPage.tsx`/`.test.tsx`. Display + Sort are unaffected. Full suite re-verified (1116 tests), typecheck clean.
- 2026-07-06: Feature request: column headers are now clickable, each opening its own dropdown (Sort ASC/DESC, Hide - omitted for Title). Replaced the standalone toolbar Sort button and `SortMenu.tsx` with this per-header control, one `usePopover` instance per header.
- 2026-07-06: Feature request: drag-to-resize for the folder panel (new `useResizablePanel` hook, pointer drag + Arrow-key, 200-480px clamp, `FolderPanel` takes an explicit `width` prop, a separator handle between the panel and the table).
- 2026-07-06: UX fixes (user-reported): the header dropdown was a fixed 200px with icon/text split apart (`justify-content: space-between`) - now `width: max-content` with left-aligned icon+text; "Sort ascending"/"Sort descending" shortened to "Sort ASC"/"Sort DESC"; the header's clickable area was only the label text, not the full cell - the trigger button now fills the whole `<th>`; a `parse-failed` row's "No metadata" chip wrapped to two lines and grew the row - now renders "-".
- 2026-07-06: Fix (user-reported): a wide column's (e.g. Title) header dropdown was anchoring off the trigger's right edge, landing far from the label over the next column - `usePopover`'s anchor now carries both `left` and `right`; the header dropdown anchors from `left`, toolbar buttons (Display) stay `right`-anchored.
- 2026-07-06: Feature request: adjustable column widths. New generic `useDragResize` primitive (pointer drag + Arrow-key, shared by `useResizablePanel` and the new `useColumnWidths`); a resize handle per column header. Found and fixed during implementation: `table-layout: fixed` + `width: 100%` rescales `<col>` widths proportionally when they don't sum to the table's rendered width, so resizing one column was visibly resizing others (mostly Title) - fixed by sizing the `<table>` itself to the exact sum of visible column widths. Also fixed the handle straddling the column border (`translateX(50%)`), which let the next column's full-cell header button steal its clicks - kept the handle fully inside its own column instead.
- 2026-07-06: Second cross-model Codex review (full branch diff since the first review, `main...HEAD`): 0 High, 4 Medium, 1 Low. Reverted the modifier-keydown blur guard added in an earlier commit - it broke Shift+Tab keyboard navigation (a real regression; the CSS `outline: none` suppression alone was already sufficient). Added a "Clear sort" item to the active column's dropdown (previously no way back to default order without a reload). Added `aria-sort` to sortable `<th>` elements for screen readers. Fixed a `useDragResize` document-listener leak on unmount mid-drag. One Medium (hiding the active sort column drops its visible caret) reaffirmed as accepted independent-view-state behavior per the first review's documented decision. Full suite re-verified (1147 tests), typecheck clean.
- 2026-07-06: PR #55 merged to `main` (squash). Story done.
