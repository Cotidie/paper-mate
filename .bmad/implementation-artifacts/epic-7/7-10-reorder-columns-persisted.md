...
- **2026-07-08:** Post-review fix-request round, driven by live user testing in the browser (not caught by unit tests or the first code review). (1) The main-area file-drop dropzone border was incorrectly showing during a column-header drag - `LibraryPage`'s `dragover` handler gated the highlight on ANY drag, not just an incoming OS file drag; fixed by checking `dataTransfer.types.includes("Files")`. (2) Added a live drag-over preview (columns swap into their would-land position WHILE dragging, not just on drop), which required switching `reorderColumns` from "insert before target" to standard array-move semantics (a forward drag lands after the target, backward lands before) - the old semantics made dragging a column onto its immediate right neighbor a no-op, the single most common gesture. (3) The live preview then oscillated severely on a REAL continuously-moving mouse (confirmed via chrome-devtools MCP real-browser testing, not just simulated events) - root-caused to element-based hit-testing being inherently unstable once the live preview reorders things under the cursor; replaced with frozen-geometry targeting (each header's screen position snapshotted once at drag start, every dragover resolved from raw `clientX` against that snapshot, never from which DOM element received the event). (4) The drop indicator always rendered on the left/"before" edge regardless of drag direction, visibly pointing at the wrong side for a forward drag; fixed by computing the indicator side from the same fromIdx/toIdx comparison `reorderColumns` itself uses. All four were caught and verified via live browser testing (chrome-devtools MCP), not unit tests alone, per two of this repo's standing engineering principles (live-smoke real interaction; measure geometry, don't assume). A second Codex `bmad-code-review` pass on the full accumulated diff found 0 High, 1 Medium (a corrupt persisted column width could bypass the resize clamp and render out of range - fixed by validating range at reconcile time, with the clamp bounds moved to `tableView.ts` to avoid a circular import), 3 Low (a `useDragResize` re-entrant-drag listener leak, and two stale comments) - all fixed with regression tests. Two further fix requests in the same round: the default column order is now Title/Authors/Venue/Year/DOI/Location/Added with File type hidden by default (was DOI hidden); an uncategorized paper's Location cell now renders empty instead of literal "Uncategorized" text (the sidebar's own "Uncategorized" folder-panel entry is unaffected). Client suite: 1342 tests passing, typecheck clean. Live-smoked again on fresh servers confirming both the real-mouse-drag fix and the two new fix requests.
