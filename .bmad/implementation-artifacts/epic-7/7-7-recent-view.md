...
- 2026-07-07 (post-Task-8 user fix request, live-smoked): the date-bucket header row (Today/Yesterday/Last week/Last month) was too easy to mistake for a data row - a faint first-pass tint still wasn't distinct enough. Escalated to the `--color-surface-strong` tone (the same one hover/selected already use), a top hairline to cap the previous group off, and `--color-ink` text at 600 weight; also fixed a real bug the review surfaced along the way - the header `<tr>` inherited the generic tbody row's hover tint + pointer cursor, so it looked clickable/selectable despite having no click handler. Neutralized with a same-background hover override and `cursor: default`.
