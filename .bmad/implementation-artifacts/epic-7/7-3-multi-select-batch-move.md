...
- [x] **Task 8 - Follow-up fix request (user-reported, second repro): stray focus ring ALSO appeared from a plain click + merely holding Shift after, no second click**
  - [x] Root cause (deeper than Task 7): Chromium's `:focus-visible` heuristic re-evaluates on ANY keydown while an element already holds focus, not only at the moment focus is first granted. A plain click arms a single row (intended: native `mousedown` focuses the tabIndex=0 Title/Authors `<td>` so Enter-to-edit works) and the ring stays hidden because that focus came from a mouse click - but then simply pressing/holding Shift (no new click at all) flips `:focus-visible` true on the SAME already-focused cell, painting the ring with no click-phase JS handler ever firing to catch it. Task 7's `handleRowClickCapture` blur fix only runs inside a click event, so it could not address this sequence.
  - [x] Fix: `CollectionTable.css` now unconditionally suppresses `outline` on `.collection-table__title`/`.collection-table__authors` for both `:focus` and `:focus-visible` - the row's own armed/checked treatment (ink bar + `{colors.surface-strong}`) is already this cell's selection indicator, so a second ring driven by incidental DOM focus timing is a redundant surprise, not a deliberate affordance, suppressed outright rather than merely relocated to a different trigger condition.
  - [x] Verified live (Playwright, own fresh servers): plain-clicked a row to arm it, pressed Shift (`page.keyboard.press`), confirmed via `document.activeElement.matches(':focus-visible') === true` (heuristic did flip) yet `getComputedStyle(...).outlineStyle === 'none'` and a screenshot shows no ring - only the intended row highlight.
  - [x] Full suite re-run green (1071 tests) + typecheck clean after the CSS change.
...
- **Follow-up fix request (second user repro, after Task 7 shipped):** the ring still appeared from a plain click (no modifier at all) followed by merely holding Shift, no second click. Deeper root cause: Chromium's `:focus-visible` heuristic re-evaluates on ANY keydown while an element already holds focus - a plain click's native `mousedown` focus is intentional (Enter-to-edit) and starts with the ring hidden (mouse-triggered), but a later bare Shift keydown on that still-focused cell flips `:focus-visible` true with zero new click, so no click-phase JS handler (Task 7's blur included) ever runs to catch it. Fixed at the CSS layer instead: `.collection-table__title`/`.collection-table__authors` now unconditionally set `outline: none` for both `:focus` and `:focus-visible`, since the row's own armed/checked ink-bar highlight is already this cell's real selection indicator. Verified live: pressed Shift after arming a row, confirmed `:focus-visible` really did flip true (`el.matches(':focus-visible') === true`) yet `getComputedStyle(el).outlineStyle === 'none'` and a screenshot shows no ring.
...
- `client/src/library/CollectionTable/CollectionTable.css` - unconditional `outline: none` on the Title/Authors cell for `:focus`/`:focus-visible` (Task 8 follow-up fix).
...
- 2026-07-06: Follow-up fix request (user-reported, second repro): a plain click + merely holding Shift after (no second click) still showed the ring, since Chromium's `:focus-visible` heuristic can flip on a bare keydown against an already-focused element. Fixed at the CSS layer with an unconditional `outline: none` on the Title/Authors cell; re-verified live (Task 8).
