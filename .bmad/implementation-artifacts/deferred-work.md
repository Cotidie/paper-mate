...
- ~~**Scroll-away render cancellation**~~ **(PROMOTED 2026-06-28 → Story 1.9)** [client/src/Reader.tsx] — `PageCard` marks a page visible once and disconnects the observer, so in-flight page renders cancel on unmount or scale change but not when the card leaves the viewport. Now tracked as Story 1.9 (render perf — windowing & viewport unification) via correct-course; see `.bmad/planning-artifacts/sprint-change-proposal-2026-06-28-render.md`.
