...
- [x] [Review][Patch] [Low] Correct Task 6(g)'s checked live-smoke claim or perform the omitted browser check [.bmad/implementation-artifacts/10-4-resizable-persisted-collapsed-memo.md:61] — **Corrected**: Task 6(g)'s checkbox text now states plainly that the comment-bubble/preview regression check was NOT live-verified in-browser (only via the green unit-test suite + a diff review confirming zero touched comment files), rather than implying a live check occurred.
...
- `client/src/annotations/gestures/useEditGesture.ts` — `DragState.collapsed`/`collapsedWidth`; `onDown` height re-seed (unconditional, Story 10.2) + width re-seed (collapsed-only); `computeAnchor` collapsed width-only re-anchor with page-edge clamp, move clamp widened by `collapsedWidth`; `onUp` collapsed commit branch (routes to `resizeCollapsedMemo`, skips `setActiveMemoSize`).
...
- `.bmad/implementation-artifacts/sprint-status.yaml` — `10-4-resizable-persisted-collapsed-memo` → `in-progress` → `review` → `in-progress` (Codex findings) → `review` (this round).
