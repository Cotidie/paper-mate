...
| 2026-06-28 | Swapped emoji for monochrome Phosphor (regular) icons (user pref) — added `@phosphor-icons/react` dep, `Cursor`/`Hand`/`Selection`/`CaretDouble*` glyphs painting via `currentColor` (body → ink when armed), sized by new `--tool-icon-size` token. Live-verified colors = design tokens. Note: this relaxes the story's original "no new dependency" guard, by explicit user request post-implementation. |
