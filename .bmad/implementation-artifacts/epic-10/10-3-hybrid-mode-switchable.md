...
4. **(Offline + determinism surfaced, not hidden; NFR-1)** Given AD-13's "deterministic + offline for born-digital PDFs" invariant, then the change **surfaces, not hides,** how hybrid relaxes it. Model weights are **baked into the image at build** (user decision: `HF_HOME` pre-populated), so hybrid stays **offline once built** and NFR-1 is satisfiable in hybrid mode too. **Scope of the bake (narrowed during implementation, code-review Low):** only the models the shipped hybrid path actually loads, i.e. the Docling layout + TableFormer weights for the born-digital `--no-ocr` path. The **EasyOCR model dir is deliberately NOT baked** (it would add weight for a path we never take), so the offline guarantee covers born-digital hybrid extraction as shipped, not a hypothetical OCR run. Enabling OCR/scanned support later must bake EasyOCR in the same change or it would hit the network on first run; OCR/scanned stays out of scope here. If hybrid is non-deterministic run-to-run, that is documented as a MODE property. **Local mode remains the deterministic + offline default**, reachable by leaving the switch unset. The default (local) is recorded here.
...
- [~] **Task 8 — Live smoke (AC: #1, #4, #6). IN-CONTAINER DOCKER SMOKE NOW DONE (2026-07-21, code-review High follow-up); ONE ITEM REMAINS: the DPR-2 `?debugStructure=1` browser overlay placement.** The `docker compose up --build` path is proven end-to-end: clean build, hybrid extraction, the local↔hybrid restart flip, offline, and CPU fallback all verified INSIDE the container. In-container GPU passthrough stays unverifiable on this host (`docker info` runtimes = `runc` only, no `nvidia-container-toolkit`) — documented, not a blocker. Isolation used: separate compose project `-p pmsmoke`, port 8899, throwaway `PAPER_MATE_DATA` under the session scratchpad (never `~/.paper-mate`), `PAPER_MATE_UID/GID=1002`; the user's own `paper-mate-paper-mate-1` on `:8801` was never touched (verified `RestartCount: 0` + a different image sha afterward).
  - [x] **Clean image build (AC #4, #5):** `docker compose -p pmsmoke build` succeeded from cold. Image **12.7 GB** (the accepted decision-A cost). The `download_models()` bake ran at build (138s) and is the ONLY network use; runtime never fetches.
  - [x] With `PAPER_MATE_STRUCTURE_MODE=hybrid`: imported TranAD (`fixtures/sample-pdfs/adtran.pdf`) **in-container**. `/api/health` → `{"structure_mode":"hybrid"}`; `/structure` → **308 elements / 28 headings**, recovering `3 METHODOLOGY 3.1 Problem Formulation` + `3.2 Data Preprocessing` — byte-for-byte the same counts as the host-level run, so the container adds no drift. The bundled hybrid server is confirmed live in-container (`localhost:5002/health` → 200). **STILL HOST-RUN:** the synthesized-ToC UI + the `?debugStructure=1` overlay at DPR 2 (needs a browser; none available in the review session).
  - [x] **Env-flip proof (AC #2, #6):** PROVEN both directions on the SAME image, no rebuild. `PAPER_MATE_STRUCTURE_MODE=local` + restart → `/health` `"local"`, **290 elements / 25 headings**, and `3 METHODOLOGY` / `3.1` / `3.2` all **MISSING** (the motivating gap returns); flip back to `hybrid` + restart → 308/28 with the sections back. Local mode also launches **no** hybrid server (loopback `:5002` refused from inside), confirming AC #5's "local pays no runtime cost".
  - [x] **Offline proof (AC #4):** PROVEN **in-container** — a `docker run --network none` container (DNS resolution confirmed dead: `gaierror` reaching `huggingface.co`) still booted the hybrid server on loopback and ran a full TranAD hybrid extraction at **full fidelity (308 elements, both sections present)** off the baked HF cache. Zero network. (The earlier host-level `HF_HUB_OFFLINE=1` run stands as the prior evidence.)
  ...
  - [x] **CPU-fallback proof (AC #8):** PROVEN **in-container** — the container ran `PAPER_MATE_STRUCTURE_HYBRID_DEVICE=cpu` on a GPU host with no GPU passed through (no nvidia runtime available at all), booted, and extracted TranAD with no CUDA-missing crash. This is the in-container mirror the host-level run previously stood in for.
  - [x] **SPA served from the image (AD-10):** `GET /` → 200, `<title>Paper Mate</title>` + `/assets/index-*.js`, i.e. the built static SPA is served same-origin from the container.
  - [x] **Boot + latency observed in-container (CPU):** boot-to-serving in hybrid mode **9s** (includes the hybrid-server model load + the readiness poll) vs a fast local-mode boot; TranAD extraction **~6s hybrid / ~4s local**. NOTE: the hybrid figure is much faster than the spike's ~37-98s warm/cold CPU estimate that sized `hybrid_timeout` at 120s. The 120s cap is therefore generous, not tight — left as-is (headroom is the safe direction), but the spike's latency numbers should be read as an upper bound, not a typical value.
  - [x] **GPU path (AC #8):** PROVEN in the host venv — `--device cuda` ran (RTX 3090), ~16s/paper vs ~37-98s CPU, same structure. In-container GPU passthrough remains **unverified** (host `docker info` shows only the `runc` runtime, no NVIDIA container runtime); documented, not a blocker.
  - [x] Tore down both smoke containers + the compose network, purged both throwaway data dirs. The 12.7 GB `pmsmoke-paper-mate` image was left on disk (reclaim with `docker image rm pmsmoke-paper-mate`). The user's `:8801` container was left running and healthy.
...
4. **Models baked at build (offline preserved).** Pre-download the Docling weights during `docker build` into the image (`HF_HOME` + `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE`), so hybrid stays offline once built (NFR-1 satisfiable in hybrid too). Build step needs network; runtime does not. **EasyOCR weights are excluded** (the server runs `--no-ocr` for born-digital papers), narrowing the offline guarantee to the shipped path, see AC #4.
...
- **SUPERSEDED 2026-07-21 (code-review High follow-up):** the Docker items below were subsequently RUN and PASSED in-container — clean 12.7 GB build, hybrid extraction (308/28, both sections), the local↔hybrid restart flip, `--network none` offline at full fidelity, and CPU fallback. See the rewritten Task 8 for the evidence. **Still NOT verified:** the DPR-2 `?debugStructure=1` browser overlay placement of the recovered headings (no browser available in the review session), and in-container GPU passthrough (this host has an RTX 3090 but `docker info` shows only the `runc` runtime, no NVIDIA container runtime, so only the host-venv GPU path is proven).
...
- **Task 8 status (honest, updated 2026-07-21 after the code-review High):** the seam is now proven **in-container**, not just host-level. `docker compose up --build` produced a clean 12.7 GB image; hybrid extraction in the container recovers `3 METHODOLOGY`/`3.2 Data Preprocessing` (308 elements, same as host); the local↔hybrid flip works on the same image via env + restart with no rebuild (local drops to 290/25 and launches no hybrid server); a `--network none` container runs hybrid at full fidelity off the baked cache; `--device cpu` works with no NVIDIA runtime present; the built SPA is served from the image. **The ONE remaining gap is the DPR-2 `?debugStructure=1` browser overlay placement of the recovered headings** — it needs a real browser at devicePixelRatio > 1, which the review session had no tool for. Per CLAUDE.md's standing rule, coordinate/overlay work is the highest-risk path and jsdom cannot see it, so this must be smoked by a human before the story is `done`. In-container GPU passthrough also stays unverified on this host (no NVIDIA container runtime); documented, not a blocker.
