...
- [ ] **Task 6 - close-out + verification.**
  - [ ] Cross-model Codex review (AE-6, AP-3): run `bmad-code-review` via `codex exec` against the story + full diff; resolve High/Med. (Use `UV_CACHE_DIR=/tmp/uv-cache` for the sandbox backend pytest, AE-7.) **Pending — run with a DIFFERENT model than the implementer (Opus 4.8); not run inline.**
...
**Pending:** cross-model Codex review (AE-6) — run with a different model than the implementer before merge.
