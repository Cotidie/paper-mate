...
- `client/src/render/textSelection.ts` (modified: added the `copy` listener + import; code-review fix added `#rangeStaysWithinTextLayers`)
- `client/src/render/textSelection.test.ts` (modified, code-review fix: added the AC-5 full-range guard regression tests)

### Review Findings

Reviewed by Codex (`codex exec` running the `bmad-code-review` workflow standalone, per CLAUDE.md's auto-review convention: story file + `baseline_commit..HEAD` diff + sprint-status path, headless). 0 decision-needed, 4 patch, 0 defer, 4 dismissed as noise. Codex independently ran `npm test`/`typecheck`/`build` and confirmed green before reviewing. All 4 patch findings verified against source (pdf.js internals, actual app DOM structure) and fixed in this session — see Completion Notes below for the fix-by-fix writeup.

- [x] [Review][Patch] Sparse selections normalize away the only large paragraph gap and real pdf.js font size is never measured [client/src/render/paragraphCopy.ts:72]
- [x] [Review][Patch] Mixed selections with text outside registered text layers pass the endpoint-only guard and lose native clipboard content [client/src/render/textSelection.ts:115]
- [x] [Review][Patch] Multi-range PDF selections are validated in full but only the first range is copied [client/src/render/paragraphCopy.ts:169]
- [x] [Review][Patch] Existing line-edge whitespace is not normalized, so a soft-wrap boundary can contain multiple spaces or defeat de-hyphenation [client/src/render/paragraphCopy.ts:117]

**Fix 1 — sparse-selection gap-blindness + broken fontSize read.** Two compounding bugs. (a) `lineHeightOf`'s `median(gaps)` is self-referential for a 2-line selection: with exactly one gap, that gap always equals its own median, so `gap > BIG_GAP_RATIO * lineHeight` collapses to `gap > BIG_GAP_RATIO * gap`, which can never be true — a real paragraph gap between exactly 2 selected lines was silently treated as a normal wrap. Fixed by only trusting the median once there are 2+ gaps (3+ lines); below that, fall back to the font-size-derived estimate, which doesn't reference the gap it's judging. (b) `measureSelectedLines` read `span.style.fontSize` (the inline style), but pdf.js's `TextLayer` (confirmed by reading `node_modules/pdfjs-dist/build/pdf.mjs`) never sets that — it sets a `--font-height` CSS custom property instead, so the read always silently returned `""` → `0`, defeating the font-size fallback in (a) even after fixing the threshold. Fixed by reading `getComputedStyle(span).fontSize` instead, which resolves the actual rendered value (confirmed live: `19.92px` on a real COCO span, vs the old code's permanent `0`). Added 2 new pure-function tests (`paragraphCopy.test.ts` (g)) proving a 2-line genuine paragraph-gap now breaks and a 2-line normal wrap still joins.

**Fix 2 — endpoint-only AC-5 guard.** The `copy` handler validated only each range's `startContainer`/`endContainer`, not everything the range actually spans — a range that starts and ends inside registered text layers could still cross non-layer content in between (document order, not visual position) without the guard catching it, silently dropping that content from the clipboard while still calling `preventDefault()`. Traced the actual Reader DOM (`PageCard.tsx`: each page is only `canvas` + `.textLayer`; `AnnotationInteraction` renders once, as a single sibling AFTER all page cards, never interposed) — not reachable in this app TODAY, but the guard logic was unsound regardless and a future story (8.4, comment-on-region) could add per-page overlays that make it reachable. Replaced with `#rangeStaysWithinTextLayers`, a `TreeWalker` (`SHOW_TEXT`, `acceptNode` filtered by `range.intersectsNode`) bounded to `range.commonAncestorContainer` that checks every text node the range touches, not just its two endpoints. Added 2 new tests in `textSelection.test.ts` (negative: a range crossing an unregistered sibling div is NOT intercepted; positive: a legitimate 2-layer selection with no interposed content IS still intercepted — proving the fix doesn't regress the supported multi-page case).

**Fix 3 — multi-range selections silently dropped ranges 2..N.** The AC-5 guard validated every range in the selection, but `measureSelectedLines` unconditionally used only `getRangeAt(0)`, so a discontiguous multi-range selection (Firefox ctrl+drag) would copy just its first range while still blocking native copy — silent data loss for the dropped ranges. Fixed with the simplest correct guard consistent with the handler's existing defensive philosophy ("fall through to native copy rather than clobber the clipboard"): bail to native copy when `selection.rangeCount !== 1`. Not unit-tested — confirmed via probe that jsdom's `Selection.addRange` only ever keeps 1 range (matches Chrome; genuine multi-range is Firefox-only), so a real multi-range `Selection` object isn't constructible in this test environment. Verified by code inspection: the one-line early-return is unambiguous and low-risk.

**Fix 4 — line-edge whitespace / hyphen-trim bug.** `HYPHEN_WRAP` tested `prev.text.trimEnd()` (the trimmed string) but the hyphen-strip `.replace(/-$/, "")` ran on the untrimmed original, so a trailing space after the hyphen defeated the strip (`"charac- "` stayed `"charac- "` instead of becoming `"characterizing"` after joining). Separately, a plain `" " + cur.text` join never accounted for pre-existing edge whitespace on either line, risking a double space. Fixed by normalizing the join boundary once: strip trailing whitespace from the accumulated text and leading whitespace from the incoming line before either the hyphen-strip or the space-join, so both paths operate on the same clean strings. Added 2 new pure-function tests ((h) trailing-space-after-hyphen de-hyphenates correctly, (i) pre-existing edge whitespace never produces a double space).

All 4 fixes' regression tests were verified to FAIL against the pre-fix code (`git stash` on just the source files, re-run, confirmed 4 failures matching the 4 bugs) and PASS against the fix — real regression coverage, not incidental passes. Full suite after all 4 fixes: 1380/1380 client tests, typecheck clean, build clean. Live-smoked on the real COCO PDF (own dev servers): the original user-reported drag still produces the exact correct output, and AC-5 (memo `<textarea>` copy) remains untouched.
