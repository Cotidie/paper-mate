...
6. **Given** the Venue (Short) cell renders, **Then** it shows `venue_short` when present, otherwise it falls back to the full `venue` (never blank when a full venue exists), and it exposes the full venue name on hover/focus via `title` (LFR-32).

7. **Given** sorting on Venue (Short), **Then** the sort key equals the displayed value (`venue_short` when present, else `venue`, else empty), so sort order matches what the cell shows; empty values sort last in either direction, consistent with the other string columns (LFR-32).
...
9. **Given** a paper imported before this story, or one whose Crossref work carried no short form and no event acronym, **Then** its Venue (Short) cell falls back to the full venue (or renders blank only when the full venue is itself blank), with no backfill or re-enrichment pass over the existing library (Crossref new-imports-only, matching Story 7.9) (LFR-32).
...
- [x] **Contract regen + docs** (AC: 4)
  - [x] Regenerate the contract: `cd server && PYTHONPATH= uv run python -m app.export_openapi`, then `cd client && npm run gen:api`. Commit `server/openapi.json` and `client/src/api/schema.d.ts`.
...
- [x] **Tests** (all AC)
  ...
  - [x] `client/src/library/columnSort.test.ts`: sorting on `venue_short` uses the short value, falls back to full when short is empty, and sorts empty last.
  - [x] `client/src/library/CollectionTable/CollectionTable.test.tsx`: the Venue (Short) column renders the short value, falls back to the full venue when short is absent, and carries the full venue in `title`; the Full column still renders the full venue and stays editable.
...
- `server/app/domain/enrich.py`: **no change**. The arXiv fallback only fires when `venue` is `None`, and it fills `venue` with an already-short value (`"arXiv"` or a `journal_ref`); leaving `venue_short` as `None` there is correct, since the Short cell falls back to that short-enough full value.
...
After the Pydantic edits: `cd server && PYTHONPATH= uv run python -m app.export_openapi` then `cd client && npm run gen:api`. Commit both `server/openapi.json` and `client/src/api/schema.d.ts`. `ExtractedMeta` is internal and stays out of the schema (no route references it); only `Doc`/`CollectionRow` surface `venue_short`.
...
- Backend: `cd server && PYTHONPATH= PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run pytest -q` → 300 passed.
- Frontend: `npm test -- --run` → 68 files / 1429 tests passed. `npm run typecheck` clean.
- Live smoke: fresh `uvicorn` on :8099 + fresh `vite` on :5199 (own servers, not the user's running instance), data root `/tmp/pm-smoke-data`. Imported `fixtures/sample-pdfs/3706598.3713941.pdf`; after enrichment settled (`status: ready`), confirmed via API and Playwright: `venue_short: "CHI"`, `venue` = full CHI 2025 proceedings title. Verified in-browser: hover title on the Short cell shows the full venue, ASC sort sets `aria-sort="ascending"` on the `venue_short` header, and the Display menu's "Venue (Short)" checkbox hides/shows the column. Also confirmed both the fresh-user default order (Venue (Short) immediately before Venue (Full)) and the append-at-end reconcile behavior for a pre-existing customized layout found in the browser profile's `localStorage`. Servers/scratch data torn down after.
...
- **Short-venue source:** Crossref's `short-container-title` is empty for the sample DOI (`10.1145/3706598.3713941`, an ACM CHI proceedings) — the short form comes from `event.acronym` ("CHI '25") with the trailing year token stripped, confirmed live in the smoke run (`venue_short: "CHI"`).
...
- **Version bump:** `server/pyproject.toml` `0.5.18` → `0.5.19` (PATCH +1, story done).
- Test-fixture note: several existing CollectionTable/LibraryPage/useTableView tests hardcoded the old single-Venue-column order/label ("Venue") or a `CollectionRow` fixture with `venue` set but no `venue_short` (causing `venue_short`'s full-venue fallback text to collide with the Full cell's text under `getByText`). Updated those fixtures/expectations in place; no production behavior changed.
