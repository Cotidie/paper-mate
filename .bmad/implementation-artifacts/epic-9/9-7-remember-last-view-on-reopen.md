...
6. **(Out of scope, resolves open design calls)** ~~Given this story, then it does NOT persist a per-page zoom LEVEL (only page + fraction)~~ — **superseded 2026-07-19 by direct user request**: the reader ALSO remembers the exact zoom scale it was at when captured (an optional `scale` field on the stored entry, degrading independently — a missing or corrupt `scale` falls back to fit-to-width, so page/frac stay the scale-independent landing mechanism regardless). It does NOT sync the position across devices (Remote sync stays deferred). The position lives in a client-only `localStorage` view-prefs store, mirroring the Story 7.10 `tableViewPrefs`/Story 5.1 `settings` stores — not in `meta.json` and not the annotation working copy.
...
- ~~Per-page ZOOM level memory~~ — **superseded 2026-07-19** (direct user request, post-review): the reader now ALSO remembers the exact zoom scale (optional `scale` field, `lastView.ts`), restored by `Reader`'s load effect reading it directly before `phase` flips to `"ready"` (so cards mount at final geometry from the first paint, same reserve-geometry invariant `restoreView` relies on). page/frac remain the scale-independent landing mechanism regardless — a missing/corrupt `scale` just falls back to fit-to-width, same reconcile discipline as every other field.
...
### Post-implementation follow-up: also remember zoom level (direct user request, 2026-07-19)

Reverses AC #6's original "no zoom memory" decision. `LastView` gained an optional `scale` field (raw pixel scale factor, mirrors `useZoomControl`'s `scale` state, not a rounded percent), captured alongside page/frac (mirrored via a `scaleRef`, same pattern as `currentPageRef`, so a zoom change doesn't tear down the scroll listener). `Reader`'s load effect reads the remembered scale directly (`useLastViewStore.getState().positions[doc.doc_id]?.scale`) and uses it as the initial `setScale(...)` value BEFORE `phase` flips to `"ready"`, instead of always `computeFitScale(...)` — so cards mount at the correct final geometry from the first paint, matching the existing reserve-geometry invariant `restoreView`'s scroll math already depends on (no flash-then-rescale). `reconcile` validates `scale` independently (finite, `> 0`; otherwise dropped, entry survives without it). Verified: typecheck clean, 4 new tests (2 `lastView.test.ts` reconcile cases + 1 store test, 1 `useRememberedView.test.tsx` scale-capture case), live-smoked at DPR 2 (391% zoom → Back → reopen → landed on the identical content at 391%; zoom captured as raw scale `3.90625`).

**Incident during this follow-up:** the `codex exec` review below was launched with `--sandbox workspace-write`; partway through its run it reset the working tree, silently discarding this follow-up's then-uncommitted edits (5 files). No git stash/reflog entry (working-tree wipes aren't ref operations). Recovered by retyping the just-made edits from conversation context, reverified (typecheck + full suite green), and committed immediately. Future Codex review invocations should use `--sandbox read-only`.

### Senior Developer Review (AI) — Codex, 2026-07-19

Ran `bmad-code-review` through **Codex** (`codex exec --sandbox workspace-write`, GPT model — different from the Sonnet implementer, per CLAUDE.md), against `baseline_commit..HEAD` (the original page/frac-only implementation, commit `1d51392` — launched before the zoom-remember follow-up above existed, so it reviewed the ORIGINAL scope only). Verdict: **Changes Requested — High 1 / Med 1 / Low 0.** AC #2-#6 pass; AC #7 accepted on the documented DPR-2 smoke record (not rerun by the reviewer). Both findings were REAL, confirmed empirically (not just plausible), and fixed:

- **[High, fixed] H1** — the unmount flush (`useRememberedView`'s cleanup) was a PASSIVE effect, but `PageCard`'s card-deregistration is a LAYOUT effect (promoted earlier in this same story, Debug Log above). Passive-effect cleanups across a whole commit run strictly after ALL layout-effect cleanups, so on a real Back-to-Library unmount, every card was ALREADY deregistered by the time the flush ran — `cards.current.get(page)` always missed, silently losing the position if the click landed inside the 400ms debounce window. My own live smoke never caught this because it always waited past the debounce before navigating away, and the unit test used a synthetic `cards` Map populated outside any real component lifecycle. Fixed: moved the flush into its OWN `useLayoutEffect` (`useLayoutEffect(() => capture, [capture])`). Verified empirically (not assumed) via two throwaway scratch tests that parent-layout-cleanup always fires before child-layout-cleanup on unmount, AND that all changed-effect cleanups in a commit fire before any new setups — both hold for this React version. Added a regression test (`useRememberedView.test.tsx`) that mounts a REAL child component registering into a shared `cards` Map via its own `useLayoutEffect` (mirroring `PageCard` exactly) and unmounts without ever advancing the debounce timer. Live-reverified: scrolled, clicked Back 150ms later (well inside the 400ms window) → position correctly persisted; reopened → landed at the exact pixel (`scrollTop` matched to the fraction) — previously this would have silently landed at page 1 or a stale position.
- **[Med, fixed] M2** — a doc switch WITHOUT unmounting (today unreachable in the live app — `ReaderPage`'s load effect sets `doc` to `null` before fetching the next one, which unmounts `Reader` — but the hook's own switch-handling code exists and needed to stay correct in isolation) had a render-phase reset (`restoredRef.current = false`, mutating a SHARED ref for the incoming doc) that ran before the outgoing doc's flush could read its own "was I restored" state, since render always precedes commit. Fixed: replaced the shared boolean with `restoredDocRef.current === docId` (a comparison, not a flag), written ONLY inside the restore effect's commit-phase setup, never at render time — so any cleanup (which always runs before any new setup in the same commit) sees the outgoing doc's still-correct history. Added a regression test asserting the outgoing doc's flush uses its OWN geometry, not geometry the incoming doc's `restoreView` had already mutated the shared scroll container to.

Both fixes verified: typecheck clean, full suite 1642 tests passing (13 in `useRememberedView.test.tsx`, up from 11), live-reverified the H1 scenario in browser.

### File List

- `client/src/reader/lastView.ts` (new; extended post-review with optional `scale` field)
- `client/src/reader/lastView.test.ts` (new; extended with `scale` reconcile tests)
- `client/src/reader/useRememberedView.ts` (new; extended with `scale` capture, then rewritten post-Codex-review for the layout-effect flush + doc-scoped restore tracking)
- `client/src/reader/useRememberedView.test.tsx` (new; extended with `scale` tests + 2 Codex-review regression tests)
...
- `client/src/components/Reader/Reader.tsx` (modified — wired `useRememberedView`; extended post-review to read remembered `scale` in the load effect)
