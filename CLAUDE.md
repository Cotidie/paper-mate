...
- Backend: **Python 3.12+ / FastAPI 0.138.x / Pydantic v2**, Uvicorn. Pydantic models are the single source of the annotation model + API contract → OpenAPI → **generated** TS client types via `openapi-typescript` (never hand-author client API types). Document-structure extraction uses **opendataloader-pdf** (Apache-2.0, Java core via its Python binding; needs a JRE in the image) at import; born-digital + offline, OCR/hybrid deferred (see the document-structure through-line + AD-13/AD-L8). PyMuPDF stays for the metadata-heuristic fallback.
...
2. **Paper Reading Helper** — inline previews triggered by clicking references in the text: `Figure N`/`Table N`, footnotes, citation markers (`[1]`, `[2]`). Plus paper metadata extraction, export of PDF-with-highlights, and a folder-based Library page. Built on the **document-structure layer** (see the through-line below): previews, a synthesized ToC, a Figures/Tables index, and metadata all resolve against opendataloader-pdf's typed elements rather than hand-rolled detection.
...
The three architectural through-lines that cut across phases:
...
- **Document structure layer (opendataloader-pdf).** We adopt **opendataloader-pdf** (Apache-2.0) as the document-structure engine and **heavily exploit it, replacing custom PDF-interpretation logic with the building blocks it provides**, not writing our own. It runs server-side at import (Java core via its Python binding, in-container with a JRE) and produces a per-doc `structure.json` of typed, box-anchored elements (heading/paragraph/table/figure/caption/list/footnote + reading order). Those boxes map 1:1 onto the spatial-anchor model, so ONE extraction pass makes the reader section-aware and feeds every downstream consumer as a thin reader: synthesized Table of Contents, a Figures/Tables index, inline reading-helper previews, and better metadata extraction (Phase 2), plus the **paper digest** that Phase-3's AI companion auto-injects as context. Two modes: **local** (deterministic + offline, born-digital) and **hybrid** (the higher-fidelity Docling backend, Story 10.3). The mode is **switchable at runtime** from the toggle at the bottom of the Library page's left panel (`app/structure_mode.py` owns it: it starts/stops the hybrid server process, snapshots the mode per extraction, and persists the choice to `settings.json` in the data dir, which outranks `PAPER_MATE_STRUCTURE_MODE` on the next boot). A switch applies to papers imported after it; nothing is re-extracted. OCR/scanned support stays deferred. Unlike the Phase-3 agent CLIs, it runs INSIDE the container (the PDF is already in `/data`), so it does NOT hit the host-exec boundary below. Architecture: **AD-13** (main spine, the structure layer) + **AD-L8** (library spine, extraction = the domain layer's second tenant); built in **Epic 10** (`extract_structure` port + adapter behind `server/app/domain/structure.py`, a `DocStructure` contract, a client `structure/` service). The heuristics it replaces (title-from-fonts, embedded-outline ToC, regex reading-helper) survive only as graceful fallbacks when structure is thin. **Prefer this tool's output over hand-rolled PDF parsing whenever building a section/structure-aware feature.**
- **Agent abstraction.** AI features call local agent CLIs, not a hosted API. Claude/Codex/Antigravity must sit behind one switchable interface so vendor selection and the default paper-digest context are vendor-agnostic. The default paper-digest context is built from the document-structure layer above (reading-order, header/footer-stripped, sectioned text), so it is tool-derived, not hand-parsed. Open architectural problem (deferred to Phase 3): a dockerized backend cannot exec host agent CLIs — the abstraction must not assume same-process exec. `server/app/agents/` reserves the seam.
...
- **MINOR +1** per epic completed. Resets PATCH to 0. (Early on minor tracks the epic number: epic 1 done → `0.1.0`; they decouple after 1.0.) **Exception (user decision 2026-07-11): Epic 7's close does NOT take a MINOR bump — it stays on `0.5.x`. Do not bump to `0.6.0` at Epic 7 close.**
