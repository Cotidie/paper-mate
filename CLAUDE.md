...
Paper Mate is a **web** PDF paper-reading companion: annotation + AI chat optimized for reading papers. The repo is **pre-implementation** — only `README.md`, `DESIGN.md`, and a BMad install exist. No application code, `package.json`, build, lint, or test setup yet. The stack has not been chosen. When you scaffold it, record the real build/test/run commands here, replacing this section.

## Product shape

Three phases, build in order (full spec lives in the Obsidian note / the prompt that bootstrapped this repo):
...
- **Agent abstraction.** AI features call local agent CLIs, not a hosted API. Claude/Codex/Antigravity must sit behind one switchable interface so vendor selection and the default paper-digest context are vendor-agnostic.
...
- `.bmad/planning-artifacts/` and `.bmad/implementation-artifacts/` — where BMad workflow outputs land.
- `.claude/skills/bmad-*` — 61 BMad skills. `bmad-help` orients you; greenfield path is product-brief → PRD → architecture → epics/stories → sprint. Run each BMad workflow in a fresh context window.
