...
- **Single-command boot:** `docker compose up` (host port `PAPER_MATE_PORT`, default 8000; data dir `PAPER_MATE_DATA`, default `~/.paper-mate` → `/data`). FastAPI serves API + built SPA from one origin (no CORS). Container runs as the host user (`PAPER_MATE_UID`/`PAPER_MATE_GID`, default 1000:1000) so `/data` files are host-owned; pre-create the data dir before first boot (`mkdir -p ~/.paper-mate`). **The image is built static: a `docker compose up` container does NOT hot-reload, so a backend edit needs `docker compose up --build`, or use the optional `compose.dev.yaml` override (`docker compose -f docker-compose.yml -f compose.dev.yaml up`) for in-container `uvicorn --reload`. Local dev = the host two-process flow above (canonical); Docker = prod-like boot. A stale container is a missing rebuild, not a bug.**
